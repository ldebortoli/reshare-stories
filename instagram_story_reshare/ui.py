...
class InstagramStoryReshareApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.publish_mode_var = tk.StringVar(value="Repost al perfil (Recomendado)")
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(story, text="Modo").grid(row=1, column=0, sticky="w", padx=(0, 8), pady=4)
        mode_combo = ttk.Combobox(
            story,
            textvariable=self.publish_mode_var,
            values=[
                "Repost al perfil (Recomendado)",
                "Repost con contenido enlazado",
                "Reupload simple (Fallback)",
            ],
            state="readonly",
        )
    ...
    def share_story(self) -> None:
        def task():
            ...
            result = client.repost_story(
                ...
                publish_mode=self._selected_publish_mode(),
            )
    ...
    def _selected_publish_mode(self) -> str:
        mapping = {
            "Repost al perfil (Recomendado)": "reupload",
            "Repost con contenido enlazado": "reupload_with_story_media_attachment",
            "Reupload simple (Fallback)": "reupload",
        }
        return mapping.get(self.publish_mode_var.get(), "reupload")
